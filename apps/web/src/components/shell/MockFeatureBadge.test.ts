...
test('mock badges distinguish persistent milestone routes from mock-backed features', () => {
  for (const path of ['/', '/docs', '/docs/one', '/mail', '/chat/general', '/dm/one', '/inbox']) {
    expect(isMockBackedPath(path)).toBeTrue()
  }
  for (const path of ['/tasks', '/tasks/one', '/tasks-trash', '/settings', '/settings/members', '/settings/sessions', '/profile']) {
