...
export function ApiTokensPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!name.trim() || !selectedProjectId || scopes.length === 0) return
    const issued = await createToken.mutateAsync({ name: name.trim(), projectId: selectedProjectId, scopes })
  ...
  return <>
    <SettingsCard title="Create API token" description="Use API tokens for bots and other integrations. The secret is shown only once.">
      <form className="api-token-form settings-grid" onSubmit={(event) => void submit(event)}>
        ...
        <div className="settings-field"><label className="field-label" htmlFor="api-token-project">Project <span className="field-required">*</span></label><Listbox id="api-token-project" aria-label="Project" value={selectedProjectId} options={projects.map((project) => ({ value: project.id, label: project.name }))} disabled={projectsQuery.isPending || projects.length === 0} onChange={setProjectId} /></div>
        ...
        <button className="button button-primary" type="submit" disabled={createToken.isPending || !name.trim() || !selectedProjectId || (!read && !write)}>Create token</button>
    ...
    <SettingsCard title="Active API tokens" description="Revoke tokens that are no longer in use." flush>
      <div className="sessions-list">
        ...
        {tokens.data?.map((token) => <div className="sessions-row" key={token.id}>
          ...
          <div className="sessions-text"><span className="sessions-device">{token.name}</span><span className="sessions-meta"><code>{token.token_prefix}…</code> · {projectNames.get(token.project_id) ?? 'Unavailable project'} · created {relativeTime(token.created_at)}</span></div>
