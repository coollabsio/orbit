  ...
  return <>
    <SettingsCard title="Create API token" description="Create a personal token, or select a non-login service account for an integration. The secret is shown only once.">
      <form className="api-token-form settings-grid" onSubmit={(event) => void submit(event)}>
        ...
        <div className="settings-field"><label className="field-label" htmlFor="api-token-expiration">Expiration</label><Listbox id="api-token-expiration" aria-label="Expiration" value={expiration} options={[...EXPIRATION_OPTIONS]} onChange={setExpiration} /></div>
        <fieldset className="api-token-scopes"><legend className="field-label">Scopes</legend>
        ...
        <label className="api-token-service-option col-span-2" data-selected={serviceAccount}>
          <input type="checkbox" checked={serviceAccount} onChange={(event) => setServiceAccount(event.target.checked)} />
          <span><strong>Use a service account</strong><span>For Discord and other unattended integrations. Tokens with the same name reuse this identity.</span></span>
        </label>
