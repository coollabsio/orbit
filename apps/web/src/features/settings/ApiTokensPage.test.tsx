...
test('admins can create a token and see its secret once', async () => {
  ...
  globalThis.fetch = mock(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (request.method === 'POST') return Response.json({ id: 'token-1', name: 'Discord bot', project_id: 'project-1', token_prefix: 'orb_12345678', scopes: ['read', 'write'], created_at: '2026-09-18T10:00:00Z', last_used_at: null, token: 'orb_123456789-secret' }, { status: 201 })
    if (request.url.includes('/projects')) return Response.json({ items: [{ id: 'project-1', workspace_id: 'workspace-1', name: 'Support', project_key: 'SUP', color: '#ffffff', version: 0 }], next_cursor: null })
    return Response.json([])
  }) as unknown as typeof fetch
  const view = renderPage('admin')
  await waitFor(() => expect(view.getByLabelText('Project').getAttribute('title')).toBe('Support'))
  ...
  expect(await request!.json()).toEqual({ name: 'Discord bot', project_id: 'project-1', scopes: ['read', 'write'] })
