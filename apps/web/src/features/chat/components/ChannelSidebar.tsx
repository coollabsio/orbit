...
const menuClass = 'rounded-lg border border-border bg-popover p-1.5 shadow-xl'
const menuItemClass =
  ...
  return (
    <div className="relative flex shrink-0 flex-col border-r border-border bg-sidebar text-sidebar-foreground max-[899px]:w-full! max-[899px]:border-r-0 max-[899px]:group-data-[view=conversation]/chat:hidden" style={{ width }}>
      ...
      <div className="relative" ref={serverMenuRef}>
        <button className="relative flex h-12 w-full items-center justify-between gap-1 border-b border-border px-3 font-semibold text-foreground/80 transition-colors hover:text-foreground" onClick={() => setServerMenuOpen((prev) => !prev)}>
          <span className="truncate text-[13px] font-semibold text-foreground">Chat</span>
          <ChevronDown className="size-3.5 shrink-0 text-muted-foreground" />
        </button>
        {serverMenuOpen ? (
          <div className={`absolute top-12 right-2 left-2 z-50 ${menuClass}`}>
            <button
          ...
          {state.chatCategories.map((cat) => {
            ...
            return (
              <div key={cat.id} className="flex min-w-0 flex-col px-2 pt-5 first:pt-3">
                <div
                  className="group flex w-full items-center px-1 pb-1"
                  onContextMenu={(e) => {
                    e.preventDefault()
                ...
                >
                  <button
                    type="button"
                    className="group/toggle relative flex min-w-0 flex-1 cursor-pointer items-center gap-1 text-xs leading-5 font-bold text-muted-foreground transition-colors select-none hover:text-foreground data-[dragging=true]:text-foreground"
                  ...
                  <button
                    type="button"
                    className="flex items-center justify-center text-muted-foreground opacity-0 transition hover:text-foreground group-hover:opacity-100"
                            ...
                            {threads.map((thread, index) => {
                              ...
                              return (
                                <button
                                  key={thread.id}
                                  type="button"
                                  className="group/row relative flex w-full min-w-0 items-center rounded-md py-1 pr-2 pl-6 text-left text-xs leading-5 font-semibold text-muted-foreground transition-colors hover:text-foreground data-[active=true]:text-foreground"
      ...
      {contextMenu
        ? createPortal(
        <div ref={contextRef} className={`fixed z-[100] min-w-44 ${menuClass}`} style={{ left: contextMenu.x, top: contextMenu.y }}>
      ...
      {renamingThread ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-xl border border-border bg-popover p-5 shadow-2xl">
            <h2 className="text-base font-semibold text-foreground">Rename thread</h2>
            <label className="mt-4 block">
              <span className="mb-1.5 block text-xs font-bold tracking-wider text-muted-foreground uppercase">Thread name</span>
              <input
                className="h-10 w-full rounded-lg border border-border bg-muted px-3 text-sm text-foreground focus:border-primary focus:outline-none"
