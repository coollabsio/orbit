            ...
            <div className="grid gap-0.5 max-[899px]:flex max-[899px]:gap-1 max-[899px]:overflow-x-auto">
              {navItems.map(({ key, label, icon: Icon, danger }) => (
                <button
                  ...
                  type="button"
                  className={cn(
                    'flex min-w-0 items-center gap-2 rounded-lg p-2 text-left text-sm leading-5 font-medium text-foreground transition-colors',
                    '[&_svg]:shrink-0 [&_svg]:text-muted-foreground',
                    'hover:bg-sidebar-accent/50 data-[active=true]:bg-sidebar-accent data-[active=true]:[&_svg]:text-primary',
                    'data-[danger=true]:text-destructive data-[danger=true]:[&_svg]:text-destructive data-[danger=true]:hover:bg-destructive/10 data-[danger=true]:data-[active=true]:bg-destructive/10',
