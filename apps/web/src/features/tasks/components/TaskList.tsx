        ...
        return (
          ...
          >
            <div className="group/hdr sticky top-0 z-[5] flex h-9 items-center gap-2 border-b border-border bg-card pr-2 pl-1.5 text-xs font-semibold text-muted-foreground transition-colors group-data-[drop-over]/section:bg-primary/10 group-data-[drop-over]/section:text-primary">
              <button
                type="button"
                className="inline-flex size-5 items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-accent hover:text-foreground"
              ...
              <button
                type="button"
                className="inline-flex size-6 items-center justify-center rounded-md text-muted-foreground/70 opacity-0 transition hover:bg-accent hover:text-foreground group-hover/hdr:opacity-100 focus-visible:opacity-100"
...
function BulkBar({
  ...
  return (
    ...
    >
      ...
      <Dropdown direction="up" trigger={() => <Button variant="ghost" className={BULK_BTN}>Status</Button>}>
        {(close) => (
          <div className={MENU}>
            {groups.map((group) => (
              <button
      ...
      <Dropdown direction="up" trigger={() => <Button variant="ghost" className={BULK_BTN}>Priority</Button>}>
        {(close) => (
          <div className={MENU}>
      ...
      <Dropdown direction="up" align="right" trigger={() => <Button variant="ghost" className={BULK_BTN}>Assignee</Button>}>
        {(close) => (
          <div className={MENU}>
            {users.map((u) => {
              const everyone = tasks.every((t) => t.assigneeIds.includes(u.id))
              return (
                <button key={u.id} className={OPTION} data-selected={everyone || undefined} onClick={() => { bulkAssign(u.id); close() }}>
                  <UserAvatar user={u} size={16} />
                  {u.name}
                  {everyone ? <X className="ml-auto size-3.5 shrink-0 text-muted-foreground/70 group-hover:text-foreground" aria-hidden="true" /> : null}
                </button>
              )
            })}
          </div>
      ...
      <Dropdown direction="up" align="right" trigger={() => <Button variant="ghost" className={BULK_BTN}>Labels</Button>}>
        {() => (
          <div className={MENU}>
            {labels.map((label) => {
              const everyone = tasks.every((t) => t.labels.includes(label.id))
              return (
                <button key={label.id} className={OPTION} data-selected={everyone || undefined} onClick={() => bulkLabel(label.id)}>
                  <span className={PILL}><span className="size-1.5 shrink-0 rounded-full" style={{ background: label.color }} />{label.name}</span>
                  {everyone ? <X className="ml-auto size-3.5 shrink-0 text-muted-foreground/70 group-hover:text-foreground" aria-hidden="true" /> : null}
                </button>
              )
            })}
