...
async function createWorkspaceAndInviteMember(page: Page, browser: Browser) {
  ...
  await invitee.getByLabel('Your name').fill('Workspace Member')
  await expect(invitee.getByLabel('Email', { exact: true })).toHaveValue('member@orbit.test')
  await expect(invitee.getByLabel('Email', { exact: true })).toHaveAttribute('readonly', '')
  await invitee.getByLabel('Password', { exact: true }).fill('member password is long enough')
