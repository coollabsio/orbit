...
fn task_problem(error: TaskError, request_id: Option<&RequestId>) -> ApiError {
    match error {
        ...
        // A Discord message is converted to a document server-side, so a rejected
        // document is a caller-content problem, reported like any other invalid field.
        TaskError::InvalidDocument { field, .. } => validation(field, request_id),
        TaskError::Conflict
