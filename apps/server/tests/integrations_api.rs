...
async fn discord_events_create_one_labeled_task_and_retries_are_idempotent() {
    ...
    // The message is stored as a rich text document. Discord's single newline stays a
    // line break inside the first paragraph; the blank line starts a second paragraph.
    assert_eq!(
        created["task"]["description_text"],
        "A very long Discord message title that needs truncation\nMore details from Discord.\nSource: https://discord.com/channels/1/2/3"
    );
    let paragraphs = created["task"]["description_json"]["content"]
        .as_array()
        .unwrap();
    assert_eq!(paragraphs.len(), 2);
    assert_eq!(paragraphs[0]["content"][1], json!({ "type": "hardBreak" }));
